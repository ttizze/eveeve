...
import { getOrCreatePageId } from './pageService'
import { getOrCreatePageVersionId } from './pageVersionService'
import { getOrCreateSourceTextId } from './sourceTextService'
import type { NumberedElement } from '../types'
import { getOrCreateTranslationStatus } from './translationStatus'
...
export async function translate(
  ...
  const translationStatus = await getOrCreateTranslationStatus(pageVersionId, targetLanguage)
...
async function translateUntranslatedElements(
  ...
  await Promise.all(extractedTranslations.map(async (translation) => {
    ...
    const sourceTextId = await getOrCreateSourceTextId(sourceText, pageId, webPageVersionId)
