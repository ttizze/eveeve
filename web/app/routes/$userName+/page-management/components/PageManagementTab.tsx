...
export function PageManagementTab({
  ...
  const getStatusBadge = (isPublished: boolean, isArchived: boolean) => {
    if (isArchived) {
    ...
    <div className="space-y-4">
      <div className="">
        ...
        <div className=" w-full py-2 flex justify-end gap-2">
          <Button variant="outline" onClick={() => setSelectedPages([])} disabled={selectedPages.length === 0}>
            Clear Selection ({selectedPages.length})
          </Button>
          <Button variant="destructive" disabled={selectedPages.length === 0}>Delete Selected</Button>
        </div>
      </div>
              ...
              <TableRow key={pageWithTitle.id}>
                <TableCell>
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => handleSelectPage(pageWithTitle.id, checked as boolean)}
                  />
                </TableCell>
                <TableCell className="font-medium">{pageWithTitle.title}</TableCell>
                <TableCell>
                  {getStatusBadge(pageWithTitle.isPublished, pageWithTitle.isArchived)}
                </TableCell>
                <TableCell>{pageWithTitle.updatedAt.toLocaleDateString()}</TableCell>
