...
const translationSchema = z.object({
  ...
  extractedNumberedElements: z.string().transform((str) => JSON.parse(str)).pipe(
    z.array(z.object({
      number: z.number(),
      text: z.string()
    }))
...
export async function translate(
  ...
  const translatedContent = await displayContent(numberedContent, allTranslations);
  const response = await fetch(`${import.meta.env.VITE_PUBLIC_API_BASE_URL}/api/save-article`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ url: url, content: translatedContent ,title: title}),
  });
  const responseText = await response.text();
  return responseText;
}

export function extractTranslations(text: string): { number: number; text: string }[] {
  
  const translations: { number: number; text: string }[] = [];
  const regex = /{\s*"number"\s*:\s*(\d+)\s*,\s*"text"\s*:\s*"((?:\\.|[^"\\])*)"\s*}/g;
